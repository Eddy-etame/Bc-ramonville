...
export function initChatbot() {
  ...
  function montrerChips() {
    ...
    /* SEULES LES ENTRÉES QUI PORTENT UN `label` DEVIENNENT DES PUCES.
       QUICKS sert deux rôles : proposer des questions à l'écran, et donner
       au bot des faits qu'il ne doit pas inventer. Une entrée peut n'avoir
       que le second — c'est le cas de « Il y a la clim ? », ajoutée le
       25/08 parce qu'en production le bot répondait « oui, la salle est
       aménagée » faute de savoir. Sans ce filtre, une entrée sans libellé
       peignait une puce « undefined » dans le panneau. */
    chipsEl.innerHTML =
      QUICKS.filter((q) => q.label)
        .slice(0, 4)
