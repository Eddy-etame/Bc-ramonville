...
4. **Vérifier les en-têtes** — une fois en ligne : `curl -I https://mmatoulouse.com` doit montrer `strict-transport-security`, `x-content-type-options`, `x-frame-options`, `referrer-policy`, `permissions-policy` et `content-security-policy`. Ils ne s'activent que sur Vercel, jamais en local.
